...
class Prod_details_model extends CI_Model {
    ...
    function get_datatables($prod_id)
    {        
        $this->_get_datatables_query();
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);

        $this->db->where('prod_id',$prod_id); // if data is part of the object by ID

        $query = $this->db->get();
        return $query->result();
    }

    function get_api_datatables($prod_id) // api function in getting data list
    {        
        $this->db->from($this->table);
    ...
    function get_duplicates($prod_id, $item_id)
    {      
        $this->db->from($this->table);
    ...
    function get_prod_detail_items($prod_id)
    {
        $this->db->from($this->table);
        $this->db->where('prod_id',$prod_id);
    ...
    function check_if_found($prod_id)
    {      
        $this->db->from($this->table);
        $this->db->where('prod_id',$prod_id);
    ...
    function count_filtered($prod_id)
    {
        $this->_get_datatables_query();
        $this->db->where('prod_id',$prod_id);
    ...
    public function count_all($prod_id)
    {
        $this->db->from($this->table);
        $this->db->where('prod_id',$prod_id);
    ...
    public function get_by_id($prod_id, $item_id)
    {
        $this->db->from($this->table);
        $this->db->where('prod_id',$prod_id);
        $this->db->where('item_id',$item_id);
    ...
    public function delete_by_id($prod_id, $item_id)
    {
        $this->db->where('prod_id', $prod_id);
        $this->db->where('item_id', $item_id);
